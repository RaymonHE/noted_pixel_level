...
// Camera config
vector<double> camera_matrix; // camera intrinsic matrix
vector<double> dist_coeffs;   // camera distortion
double width;
double height;

// Calib config
bool use_rough_calib;  // Activate if initial calibration is undesirably accurate
string calib_config_file;  // differ indoor and outdoor
// instrins matrix
...
int main(int argc, char **argv) {
  ros::init(argc, argv, "lidarCamCalib");  // initialize a node
  ros::NodeHandle nh;
  ros::Rate loop_rate(0.1);
  // Pass the values of the parameters in the yaml file to the variables defined above
  ...
  //Read the image file and the LiDAR point cloud, extract image edges by canny operator
  //build voxels, and extract LiDAR point-cloud edges simultaneously
  Calibration calibra(image_file, pcd_file, calib_config_file); // Calibration class is declared in lidar_camera_calib.hpp
  calibra.fx_ = camera_matrix[0]; // fx and fy are the pixel focal length
  calibra.cx_ = camera_matrix[2]; // cx and cy are the principal point offest
  calibra.fy_ = camera_matrix[4]; // https://ksimek.github.io/2013/08/13/intrinsic/
  calibra.cy_ = camera_matrix[5];
  calibra.k1_ = dist_coeffs[0]; // distortion coefficients
  calibra.k2_ = dist_coeffs[1]; // https://docs.opencv.org/4.x/dc/dbb/tutorial_py_calibration.html
  calibra.p1_ = dist_coeffs[2];
  calibra.p2_ = dist_coeffs[3];
  calibra.k3_ = dist_coeffs[4];
  // get the eular angle and initial translation form initial rotational matrix
  ...
  cv::imwrite("/home/raymon/Downloads/calibData/init.png", init_img);
  cv::waitKey(1000);
  // Determine whether to perform a rough calibration
  if (use_rough_calib) {
